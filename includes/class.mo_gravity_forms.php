	...
	public function mo_get_form_field_mapping_view($form_id, $form_field_mappings) {
		...
		foreach ( $form_field_mappings as $k => $v ) {
			if ($formFieldMappingArr) {
				$table .= '<tr><td>' . $v . '</td><td><img src="' . plugins_url () . '/'.mo_landing_pages_plugin::MO_LP_DIRECTORY.'/images/move.png" /></td><td><input type="text" name="gfffm[' . $k . ']" value="' . $formFieldMappingArr [$k] . '" /></td></tr>';
			} else {
				$table .= '<tr><td>' . $v . '</td><td><img src="' . plugins_url () . '/'.mo_landing_pages_plugin::MO_LP_DIRECTORY.'/images/move.png" /></td><td><input type="text" name="gfffm[' . $k . ']" /></td></tr>';
			}
		}
		$table .= '</tbody>';
		$table .= '</table>';
