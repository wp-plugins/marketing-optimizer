...
class mo_sp_post_type {
	...
	public function mo_sp_add_variation_cookie_js() {
		...
		if ($post->post_type == 'mo_sp' && $this->mo_sp_track_admin_user () && ! $mo_sp_obj->mo_bot_detected () && ! isset ( $_GET ['preview'] )) {
			echo '<script>
	...
	public function mo_sp_get_mo_website_tracking_js() {
		...
		if ($post->post_type == 'mo_sp') {
			...
			if ($mo_settings_obj->get_mo_account_id ()) {
				$mo_sp_obj = mo_squeeze_pages::instance ( $post->ID );
				if ($mo_settings_obj->get_mo_lp_cache_compatible () == 'false' || isset ( $_GET ['mo_sp_variation_id'] ) || isset ( $_GET ['t'] ) || count ( $mo_sp_obj->get_variation_ids_arr () ) == 1) {
					...
					if (! $mo_sp_obj->mo_bot_detected () || $this->mo_page_track_admin_user ()) {
